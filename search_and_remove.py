...
# Include mode in the log filename so dry-run and real runs never overwrite each other
    ...
    try:
        ...
        for file_name in TARGET_FILES:
            try:
                ...
                if e.status == 404:
                    # File simply does not exist in this repository — skip silently
                    pass
                else:
    ...
    try:
        ...
        for file_name, line_numbers in repo_hits[repo_name].items():
            try:
                ...
                # Remove every line that contains an addShortText(...) call.
                # SEARCH_PATTERN matches the full line (including its indentation),
                # replacing it with an empty string. A trailing newline is left by
                # the preceding line, so the file does not gain blank lines.
